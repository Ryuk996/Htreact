...
export default function ImageFiles(props) {
  ...
  const [imgfilesList, setImgFilesList] = useState([]);
  const [filteredfiles, setFilteredfiles] = useState([]);

  useEffect(async () => {
    try {
      let file = await axios.get(`${env.api}/file/getpic`, { headers: { Authorization: token } })
      setImgFilesList(file.data)
    ...
    <>
      ...
      <div className="container">
        <div className="FileContentZ">
          ...
          <br></br>
          <Items filesList={imgfilesList}></Items>
        </div>
      </div>
    </>
  );
